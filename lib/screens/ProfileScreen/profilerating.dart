...
class _ProfileRatingState extends State<ProfileRating> {
  ...
  void initState() {
    ...
    Future.microtask(() async {
      profileRatingModel = await context.read<UserViewModel>().getRating(widget.id);
    });
    super.initState();
  }
      ...
      title: Column(
        children: [
          Row(
            children: [
              Text(
                Jiffy.parse(data.createdAt != null ? utcToLocal(data.createdAt!) : data.createdAt.toString()).fromNow(),
          ...
          Row(
            ...
            children: [
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    if (data.reviewer != null && data.reviewer!.activeSubscriptionPlanName != null) ...[
                      SizedBox(
                        height: 15,
                        child: Image(
                          image: AssetImage(data.reviewer!.activeSubscriptionPlanName.toString().contains("lord")
                              ? 'images/vector.png'
                              : data.reviewer!.activeSubscriptionPlanName.toString().contains("god")
                                  ? "images/godstutspic.png"
                                  : 'images/kingstutspic.png'),
                        ),
                      )
                    ],
                    const SizedBox(width: 5),
                    if (data.reviewer != null ? data.reviewer!.isSubscriptionActive ?? false : false) ...[
                      const Icon(
                        Icons.verified,
                        size: 15,
              ...
              Row(
                children: [
                  MyCustomRatingView(ratinigValue: double.parse(data.rating ?? "0"), fontcolor: const Color(0xFF767680)),
                ],
              ),
            ],
          ),
