...
class EditBioScreen extends StatefulWidget {
  ...
  EditBioScreen({Key? key, required this.isMyProfile, required this.id, required this.viewProfileModel}) : super(key: key);
...
class _EditBioScreenState extends State<EditBioScreen> {
  ...
  String actualGender = '';
  String actualDob = '';
  String actualCountryVal = '';
  String actualStateVal = '';
  String actualCityVal = '';
  ...
  void initState() {
    ...
    if (provider.listDummyProfesstions.isNotEmpty) {
      ...
      professionList.insert(0, ProfessionModel(id: null, profession: "Profession"));
    ...
    setState(() {
      ...
      if (widget.viewProfileModel.data!.profile!.gender != null && widget.viewProfileModel.data!.profile!.gender != "") {
        gender = toBeginningOfSentenceCase(widget.viewProfileModel.data!.profile!.gender)!;
      }

      actualGender = gender;

      if (widget.viewProfileModel.data!.profile!.dob != null && widget.viewProfileModel.data!.profile!.dob != "") {
        dateofbirthController.text = DateFormat('yyyy-MM-dd').format(DateFormat("yyyy-MM-dd").parse(widget.viewProfileModel.data!.profile!.dob!));
      }

      actualDob = dateofbirthController.text;

      countryValue =
          widget.viewProfileModel.data!.profile!.country != null ? toBeginningOfSentenceCase(widget.viewProfileModel.data!.profile!.country!)! : "";
      actualCountryVal = countryValue;
      stateValue =
          widget.viewProfileModel.data!.profile!.state != null ? toBeginningOfSentenceCase(widget.viewProfileModel.data!.profile!.state!)! : "";
      actualStateVal = stateValue;
      cityValue = widget.viewProfileModel.data!.profile!.city != null ? toBeginningOfSentenceCase(widget.viewProfileModel.data!.profile!.city)! : "";
      actualCityVal = cityValue;
      address.text = widget.viewProfileModel.data!.profile!.address ?? "";
      actualAddress = address.text;
    });
  }
              ...
              return CustomButton(
                () async {
                  if (_form.currentState!.validate()) {
                    ...
                    payload.addIf(((actualName != name.text.trim()) && name.text.trim().isNotEmpty), "nickname", name.text.trim());
                    payload.addIf(((actualDob != dateofbirthController.text.trim()) && dateofbirthController.text.trim().isNotEmpty), "dob",
                        dateofbirthController.text.trim());
                    payload.addIf(((actualGender != gender) && gender != "Gender"), "gender", gender);
                    payload.addIf(((actualCountryVal != countryValue) && countryValue != ""), "country", countryValue);
                    payload.addIf(((actualStateVal != stateValue) && stateValue != ""), "state", stateValue);
                    payload.addIf(((actualCityVal != cityValue) && cityValue != ""), "city", cityValue);
                    payload.addIf(((actualAddress != address.text.trim()) && address.text.trim().isNotEmpty), "address", address.text.trim());

                    if (selectedProfession.profession != "Profession") {
                      provider.editOtherProfession(widget.id, selectedProfession.id.toString());
                    }
                ...
                text: provider.isEditSocialMedia ? "loading" : "Save Changes".tr,
                color: primaryColor,
  ...
  Future<void> _selectImage(bool isFromCamera) async {
    if (isFromCamera) {
      ...
      if (sizeInMB > 3) {
        UIHelper.showMySnak(title: "Media", message: 'Maximum allowed size is 3 MB', isError: true);
        pickedFile = null;
      }
    } else {
      ...
      child: Stack(children: [
        ...
        Positioned(
          ...
          child: Container(
            width: 30,
            height: 30,
            decoration: BoxDecoration(border: Border.all(), shape: BoxShape.circle, color: whiteColor),
            child: IconButton(
  ...
  void myDateBirthPicker(context, onPick) {
    ...
    DatePicker.showDatePicker(
      ...
      pickerTheme: DateTimePickerTheme(itemTextStyle: textStyleWorkSense(color: Colors.black, fontSize: 16.0)),
      ...
      children: <Widget>[
        ...
        CSCPicker(
          ...
          dropdownDecoration: BoxDecoration(
              borderRadius: const BorderRadius.all(Radius.circular(5)), color: Colors.white, border: Border.all(color: Colors.grey, width: 1)),

          disabledDropdownDecoration: BoxDecoration(
              borderRadius: const BorderRadius.all(Radius.circular(5)), color: Colors.white, border: Border.all(color: Colors.grey, width: 1)),

          currentCountry: widget.viewProfileModel.data!.profile!.country != null
              ? toBeginningOfSentenceCase(widget.viewProfileModel.data!.profile!.country!)
              : null,
          currentState:
              widget.viewProfileModel.data!.profile!.state != null ? toBeginningOfSentenceCase(widget.viewProfileModel.data!.profile!.state!) : null,
          currentCity:
              widget.viewProfileModel.data!.profile!.city != null ? toBeginningOfSentenceCase(widget.viewProfileModel.data!.profile!.city!) : null,
