...
class _AddressViewState extends State<StateCountryOtherView> {
  ...
  //         (address.address!.country == null || address.address!.country!.isEmpty));
  //     if (widget.nickName.isNotEmpty) {
  //       String formattedAddress = [widget.nickName.first.address!.city, widget.nickName.first.address!.state, widget.nickName.first.address!.country]
    ...
    getUserData().then((data) {
      ...
      for (var nick in widget.nickName) {
        if (nick.addedBy != null && nick.addedBy!.toLowerCase().contains(userName.toLowerCase())) {
          matchedCountry = nick.address?.country;
          break;
        }
      }
      widget.nickName.removeWhere(
        (element) => element.address?.country == null || element.address!.country!.trim().isEmpty,
      );

      setState(() {
        loggedinUser = userName;
        // Use matched email if found, otherwise use the first one
        // selectedName = matchedCountry ?? widget.nickName.first.address?.country ?? "";
        ...
        : widget.nickName.isEmpty
            ...
            : PopupMenuButton(
                ...
                child: Row(
                  ...
                  children: [
                    Container(
                      constraints: BoxConstraints(minWidth: 0, maxWidth: Get.width * 0.62),
                ...
                itemBuilder: (context) {
                  return widget.nickName.map((e) {
                    String formattedAddress =
                        [e.address!.city, e.address!.state, e.address!.country].where((part) => part != null && part.isNotEmpty).join(', ');
